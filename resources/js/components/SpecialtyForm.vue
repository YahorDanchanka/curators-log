  ...
  <component class="form" :is="props.component" greedy @submit.prevent="emit('submit')">
    <div class="form__body">
      ...
      <q-input
        ...
        :rules="[() => validated.error?.details.find((item: any) => item.context.key === 'code')?.message]"
        hide-bottom-space
      />
      <q-input
        class="form__control"
        ...
        :rules="[() => validated.error?.details.find((item: any) => item.context.key === 'prefix')?.message]"
        hide-bottom-space
      />
    </div>
...
const modelValue = defineModel<SpecialtyFormModel>({ required: true })
const emit = defineEmits(['submit'])

const schema = Joi.object({
  name: Joi.string().required().max(255),
  code: Joi.string().required().max(255),
  prefix: Joi.string().required().max(255),
})
