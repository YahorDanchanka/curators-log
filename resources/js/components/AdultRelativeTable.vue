<template>
  <q-table :rows="props.relatives" :columns="columns" :rows-per-page-options="[0]">
    <template v-slot:top="data">
      <div class="q-table__title">Совершеннолетние родственники</div>
      <q-space />
...
const columns = [
  ...
  },
  {
    ...
    align: 'left',
    sortable: true,
    field: (row: RelativeModel) => row.pivot?.type,
  },
  {
    name: 'job',
    label: 'Место работы',
    align: 'left',
    sortable: true,
    field: (row: RelativeModel) => row.job,
  },
  {
    name: 'position',
    label: 'Должность',
    align: 'left',
    sortable: true,
    field: (row: RelativeModel) => row.position,
  },
  {
    name: 'phone',
    label: 'Телефон',
    align: 'left',
    sortable: true,
    field: (row: RelativeModel) => row.phone,
  },
  {
    name: 'address',
    ...
    align: 'left',
    sortable: true,
    field: (row: RelativeModel) => row.address?.address,
  },
]
</script>
